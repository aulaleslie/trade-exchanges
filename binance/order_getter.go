...
func (og *OrderGetter) GetOpenOrders(ctx context.Context) (res []exchanges.OrderDetailInfo, err error) {
	...
	if err != nil {
		err = errors.Wrap(err, "error get binance open order")
		return
	}
...
func (og *OrderGetter) GetHistoryOrders(
	...
	// Need to split because on listOrdersService doesn't support filter by clientOrderID
	if orderID != nil || clientOrderID != nil {
		orderService := og.client.NewGetOrderService().Symbol(symbol)
		if orderID != nil {
	...
	orders, err := og.client.NewListOrdersService().Symbol(symbol).Do(ctx)
	if err != nil {
		fmt.Println(err)
		return nil, err
	}
